...
- **create（dry-run + `--live` gated submit）** — `create --dry-run` で R6 body（Style / Lyrics / Exclude / model / vocal gender / **V6 の Variety・Max Mode**、weirdness・style influence・audio influence slider、persona、cover 等）を構築・検証できる。通常の `create --live` は null CAPTCHA fields で送信し、Suno が CAPTCHA を要求した場合は `blocked_captcha`（exit 31）で停止する。明示 token/provider は advanced fallback であり、自動 browser retry はしない
